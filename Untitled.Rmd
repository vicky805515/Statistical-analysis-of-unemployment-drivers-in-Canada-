...
```
```{r}
# Recode SEX variable: assuming 1 = Male and 2 = Female, we recode it as 0 and 1
data$SEX_binary <- ifelse(data$SEX == 2, 1, 0)  # 1 for Female, 0 for Male
```

```{r}
# Run logistic regression with the binary SEX variable
model <- glm(SEX_binary ~ FINALWT, data = data, family = binomial)
```
...
library(ggplot2)
ggplot(data, aes(x = FINALWT, y = SEX_binary)) +
  geom_point(alpha = 0.5) +
  geom_smooth(method = "glm", method.args = list(family = "binomial"), se = FALSE) +
  labs(title = "Logistic Regression: SEX (binary) predicted by FINALWT", 
       x = "Weight", y = "Sex (0 = Male, 1 = Female)")
```
